...
def create_names_pdf(names):
    ...
    name_area_width, name_area_height = 11 * inch, 1.95 * inch
    margin_top, margin_left = 0.2 * inch, 0.4 * inch
...
def create_numbers_pdf(numbers):
    ...
    number_area_width, number_area_height = 5.9 * inch, 7.8 * inch
    margin_top, margin_left = 0.1 * inch, 0.2 * inch
    ...
    for i, number in enumerate(numbers):
        if i % max_numbers_per_page == 0 and i != 0:
            c.showPage()
        x = margin_left + (i % 2) * (number_area_width - 0.4 * inch)
...
# Streamlit UI
st.title('Name Number Generator')
